...
const bindingMap: Record<string, (data: OverlayRenderData) => string> = {
  ...
  "comment.name": (data) => data.viewer?.name ?? "Viewer",
  "comment.username": (data) => data.viewer?.username ? `@${data.viewer.username}` : "",
  "comment.avatar": (data) => data.viewer?.avatar ?? "",
  "comment.text": (data) => data.comment?.text ?? "",
  "comment.createdAt": (data) => data.comment?.createdAt ?? "",
  "gift.name": (data) => data.gift?.name ?? "",
  "gift.amount": (data) => String(data.gift?.count ?? ""),
